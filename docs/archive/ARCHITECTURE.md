...
> **历史快照，不是当前产品或发布文档。** 本文保留早期架构判断，工具数量、Web 能力、agent 后端、测试数字与目录结构均可能已过时。当前事实以 [`../STATUS.md`](../STATUS.md) 为准，未来工作以 [`../ROADMAP.md`](../ROADMAP.md) 为准。

本文当时的一句话定位是：**LangGraph 跑图，YAML 是图的真相，skill + MCP 是操控入口，Web 只做实时观测。** 其中“Web 只做实时观测”已被后续实现取代：当前 Web 还可启动、审批、恢复、删除终态运行和管理本地配置；工作流定义与画布仍只读。

如需早期背景，可结合仓库根目录 [`README.md`](../../README.md) 阅读；不要把本文中的旧接口或旧测试状态当作当前合同。
