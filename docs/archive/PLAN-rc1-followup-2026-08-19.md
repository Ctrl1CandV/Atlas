...
REVIEW-004 独立复核通过。阶段 D 已于 2026-08-19 经所有者再次确认后执行完毕：
全部经 MCP stdio 真实运行，发现并修复两个 agent 生产 blocker（CLI 契约正则、
CLI 用户 settings 劫持端点），并发生一次 Kiro sonnet-5 超支事件（单次尝试
$10.5，已强制终止，见 §7 实施记录）。** 本文最初定稿于 2026-08-18，并于
2026-08-19 按当前源码重新审计。剩余发布工作是 tag、产物上传与 provenance，
由所有者自行执行；公开仓库已按所有者决定移除开发产物（docs/tests/.github/
scripts），远端 CI 因此不在公开仓库运行；阶段 E 不在本轮。
实施过程如需偏离本文，先改本文再改代码。
...
### 7.1 阶段 D 执行记录（2026-08-19，所有者当日再次确认后执行）

执行方式：全部操作经真实 MCP stdio 协议（`uv run atlas-mcp`，官方 SDK 客户端）；
每条真实运行先 dry_run 并以 `expected_execution_sha256` 钉住预演身份；本地
pricing.json 全部单价为 null（按配置文件纪律不填猜测数字），因此 LLM 节点成本
如实记 unknown，agent 成本按 CLI 实报金额入账。结果明细在
`runs/stage_d_results.jsonl`。

**发现并修复两个生产 blocker（均在 agent 真实执行路径，此前只被桩测试覆盖）**：

1. `_check_cli_contract` 的 rf 字符串误写双反斜杠（`[\\s,=<]` 在 raw 字符串里
   是"反斜杠/s/逗号/=/<"），真实 `claude --help` 的"参数后跟空格再接描述"排版
   被误判 9 个必需参数缺失，所有真实 CLI 一律 fail-closed 拒绝——阶段 A 的
   真 CLI 核对是手工进行的，测试桩从未按真实排版走过该检查。已修复为单反斜杠
   并新增"真实排版必须通过"回归（双向验证：旧代码上新测试失败）。
2. Claude CLI 用户 settings（`~/.claude/settings.json` 的 env 块）优先于进程
   环境变量：本机个人网关配置把 agent 调用静默改道到另一端点与凭据（实测请求
   被发往 `rightapi.ai` 而非所选供应商），等于绕过 Atlas 的供应商/凭据选择。
   修复为每次调用注入空 `CLAUDE_CONFIG_DIR`（用后清理），强制 CLI 只用注入的
   端点与密钥；回归断言隔离与清理。

**示例矩阵结论（6 图 × 3 供应商）**：

- human-approval-pipeline：三供应商全部 done（flash/doubao-lite/sonnet-5）。
- map-reduce：Deepseek done（需 `max_output_tokens≥16000`；默认 8192 时 flash
  的 thinking 耗尽输出配额返回空文本，被假成功检测拦截后大声失败——引擎行为
  正确）；SuperAI done；Kiro 全系模型 thinking 能力为 none，零成本拒绝（守卫
  正确，该图在 Kiro 下结构性不可配）。
- multi-vendor-debate-judge：Deepseek done（16k）；SuperAI done；sonnet-5 判决
  输出非合法 JSON 被必填字段校验拦截，换 gpt-5.6-luna 后 done。
- parallel-research：三供应商 done（Deepseek 需 16k）。
- proposal-review-repair-loop：SuperAI done；Deepseek flash/pro 均循环未收敛被
  `max_iterations=2` 拦停（守卫正确）；Kiro sonnet-5 非 JSON、luna 8k/16k 均截断。
- code-change-review-approve：SuperAI（doubao-lite）完整成功——副本隔离（原
  demo-project 字节不变）、两轮修复循环、审批记录含 baseline/result/patch 三
  摘要、diff 产物在位、实际成本 $0.958；Deepseek flash 30 分钟未完成被整图
  deadline 终止；Kiro sonnet-5 见超支事件。
- 跨厂商备用链实测：map-reduce 以 flash 主 + doubao-lite 备（默认 8192），
  lens 两节点降级到备用模型后 run done——降级链在真实供应商下生效。
- 自定义图（并行三分支 + 汇合 + 有界修复循环 + 人工门 + coding_agent）：
  经 `atlas_save_workflow` 保存→dry_run→真跑→Web 批准，一轮收敛 done，
  实际成本 $0.044。
- 失败路径：费用帽在第二次派发前以 CostExceeded 拒绝（P0min 未知费率保守占用
  生效）；节点 1s 超时以 APITimeoutError 大声失败；进程环境注入哨兵坏密钥得
  到 401 清晰报错且无真实密钥泄漏（`config/.env` 未读未改）；对 done 运行调用
  `atlas_resume_run` 被正确拒绝。

**超支事件（如实记录）**：code-change × Kiro claude-sonnet-5 首次 agent 尝试
38 轮、CLI 自报 `total_cost_usd: 10.508`，失败后图的 `retry: 1` 自动开始第二
次尝试，运行约 6 分钟后被强制终止（进程树 taskkill，金额未知）。根因：示例图
无 `max_cost_usd` 且 pricing 全 null，美元守卫与 CLI `--max-budget-usd` 均不
生效；sonnet-5 多轮 agent 成本远超历史建议帽（总计 $2/单 run $0.5）。教训：
无费率配置时，高价模型 × 大轮次 agent 无任何成本上限；正式使用前应先填
pricing 或给图配帽。修复后的测试基线为 427 passed（新增 CLI 契约回归）。

**评级**：引擎与控制面达到预期（守卫、假成功检测、循环上限、恢复准入、降级、
审批摘要绑定、副本隔离、成本结算与 unknown 诚实全部按设计工作，并拦下两个
真实发布 blocker）；供应商适配部分达到（flash 默认 8192 易空输出/截断、
sonnet-5 结构化输出不稳、luna 长输出截断、Kiro 全系无 thinking、flash 跑大
agent 任务 30 分钟超时）；成本治理部分达到（无 pricing 时确无美元守卫是配置
事实而非缺陷，agent 实际金额入账正确）。
