...
Atlas exposes its six MCP tools two ways:
...
A git clone ships a generic [`.mcp.json`](../.mcp.json) at the repo root, so opening the cloned repository in Claude Code (or any harness with project-level MCP config) auto-starts the six tools over stdio — no manual terminal needed. The configured command is `uv --directory . run atlas-mcp`, working directory = repo root; complete `uv sync --locked --all-groups` first. The published v0.1.0 sdist does not include this file; the next release's manifest does.

Git clone 自带仓库根目录的通用 [`.mcp.json`](../.mcp.json)：用 Claude Code（或其他支持项目级 MCP 配置的 harness）打开克隆下来的仓库，六个工具会通过 stdio 自动启动，无需手动开终端。配置执行的是 `uv --directory . run atlas-mcp`，以仓库根目录为工作目录；先完成 `uv sync --locked --all-groups`。已发布的 v0.1.0 sdist 不含该文件；下一版清单已补入。
