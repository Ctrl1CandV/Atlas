# 设计:节点间通讯文件(node I/O files)

状态:设计稿,待主人确认后实施。
提出者:主人(第五轮优化)。
问题原话:"目前节点之间的输入和输出的限制很高,如果用纯文本的形式进行传输,
传输的信息量和形式会大大受损,我认为需要支持一个临时文件的功能,
这个临时文件只为通讯而存在,即方便节点之间的输入和输出。"
...
3. `coding_agent` / `research` 在隔离副本里生成的文件,可以**声明为产物**回流给下游。
...
#### (b) `collect_files`:agent 节点把副本里的文件声明为产物

`coding_agent` / `research` 在隔离副本里本来就能写文件,现在允许显式回流:
...
MCP 侧同步:`atlas_run_workflow` 增加 `attachments` 参数,支持传本机路径
(读入落盘为产物,原文件只读)。
...
- attachment 内容由本机用户提供,落盘前算哈希,进账本;不解析、不执行。
...
### 阶段 B:agent 产物回流
1. `spec.py`:加 `collect_files`(仅 agent 类型)+ 路径安全校验。
...
并在 `docs/archive/VERIFICATION.md` 记录证据。付费真跑只在阶段 A 完成后做一次
