...
- 公开 CI（ci.yml）在 `main` 三 job 全绿（E-3 起含 web-dist-smoke 冒烟 job）；README 挂 CI 徽章；`release` environment 必须人工批准（required reviewer 已配置）。
...
| Release 包含 built frontend ✅ 已实施（2026-08-27，E-3） | 让使用者免 Node；Git 仍不跟踪 `web/dist`；manifest 哈希与排除名单机检进 `atlas/distbundle.py`，细节见 [`PLAN-stage-e-2026-08-27.md`](PLAN-stage-e-2026-08-27.md) E-3 章 | clean machine 解压即可启动，frontend hash 进入 manifest/provenance |
| OS-level sandbox ✅ 调研已交付（2026-08-28，E-5） | 调研 Windows Sandbox 或 WSL backend；与 `local_cli` 并列而非把副本改名为沙箱。**结论：对天真 `wsl -e` spawn 模型 NO-GO**——取消级联红（硬杀客户端后 setsid 孤儿存活），G1 启动/G2 文件 IO 绿；细节与实测记录见 [`RESEARCH-os-sandbox.md`](RESEARCH-os-sandbox.md)，缓解路径留待独立立项 | 威胁模型四象限 + WSL2 spike ≥8 条实测 + 预定义 GO/NO-GO 判据（git 时序可验证）+ docs contract 反宣传 grep 断言——全部交付 |
| Browser matrix ✅ 已实施（2026-08-28，E-4 冒烟 + 完整矩阵） | 主题、键盘调栏、200% 缩放、系统 Edge/Chromium。已交付：键盘审批流 × Chromium/Edge/Firefox + 终局卡片 dark/light × 100%/200% 四组合基线截图 + optional e2e-smoke workflow，细节见 [`PLAN-stage-e-2026-08-27.md`](PLAN-stage-e-2026-08-27.md) E-4 章 | 可重复 GUI 测试与真实截图，不用源码字符串代替渲染验证（已达标：两连跑逐字节一致、CI 全绿） |
