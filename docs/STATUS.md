...
| 浏览器矩阵（E-4，冒烟+完整矩阵） | Playwright e2e（`e2e/` 独立 npm 包，`npx playwright test`，5 用例）：`helpers/server.py` 以 FakeProvider registry_factory 真实执行预种——每个浏览器项目一个独立暂停 gate run（批准推进终态后不饿死其他浏览器）+ 一个 done run（归一 ts/duration_s 使终局卡片数字跨次确定），registry 全程零真实供应商调用；键盘审批流跑 Chromium/Edge/Firefox 三浏览器：Tab 焦点环按 DOM 序可达 批复说明→批准→要求修改→驳回，焦点可见断言（输入框 box-shadow 环、按钮 outline ≥2px），Shift+Tab 回位→keyboard.type 批复→Tab→Enter 等价点击续跑到终局卡片，全程禁 mouse.*、选择器只用 role/text/aria；终局卡片基线截图 dark/light × 100%/200% 四组合入仓 `e2e/__screenshots__/`（100%=1280×800，200%=CSS 视口减半 640×400，用户裁定；主题经 colorScheme 锁定；字体锁定按裁定条件触发——CI 已证渲染逐字节一致，出现漂移才引入内嵌字体；截图仅维护在 chromium 项目，Edge/Firefox 只跑键盘流），locale zh-CN + 时区 Asia/Shanghai + reduce-motion 锁定，两连跑逐字节一致，容差保持默认未调，不一致必须根因修复；`e2e-smoke.yml` optional workflow（workflow_dispatch + release 触发，不并主双绿门，失败上传 traces 与截图 diff） |
