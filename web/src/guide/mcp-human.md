...
1. `atlas_validate_workflow`：校验 YAML 或保存的 workflow id。
...
5. `atlas_get_run`：查询已创建运行的状态与产物位置。

固定顺序是 validate → save（需要时）→ dry-run → 人工确认 → run → get-run。不要跳过零成本预演。

## Human 节点

`human` 节点在 Web 界面暂停，等待批准或驳回。批准记录可被下游消费；驳回应终止相应路径。人需要审阅真实上游输出，不能把节点标题当成证据。

当前指南不承诺旧版“resume”命令或接口。只能使用当前 UI 暴露的审批动作和五个 MCP 工具。
