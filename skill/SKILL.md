...
`agents.json` runner config may declare a read-only collect list (E-2B): `"collect": [{"pattern": "patches/*.patch", "name_prefix": "patch", "role": "output", "ext": ".patch"}]` — after a successful CLI run the execution directory is scanned (relative glob only: no `..`, no absolute paths, no backslashes; symlink/junction entries are refused, never followed) and matched files are stored write-once as extra artifacts appended to `node_done.artifacts` in relative-path lexicographic order. Closed roles: `output|raw|report` (`diff` belongs to the system diff collector; `error/changes/input` are not open). Logical names are `{name_prefix}.{sanitized relative path}` (path separators and dots fold to hyphens; unicode letters survive, so CJK filenames work — such names are viewable and approvable but the ASCII-only bare-name consume check cannot reference them). Hard caps: ≤20 files, ≤16 MiB per file (the artifact cap), ≤64 MiB total; exceeding any fails the node as a governance error with counts and bytes in the message — lists are never silently truncated, and two relative paths that sanitize to the same logical name fail loudly instead of silently shadowing each other. System-excluded directories (`.git`, `node_modules`, `.venv`, `dist`, `build`, `__pycache__`, `.trash`) can be extended via `collect_exclude_dirs` but never shrunk. Collect is local_cli-only; read-only agents may collect (scanning is read-only), the research temp directory is reported by the runner, and a runner that reports no execution directory skips collection with a loud ledger note instead of pretending. Collection adds no model cost; collected artifacts replay through `node_done` like any other artifact, and a downstream node may consume one by its bare logical name.
