...
- `allowed_paths` is accepted only for `research` or `coding_agent` with `writable: false`. Writable coding plus `allowed_paths` fails before run creation: Claude `--add-dir` grants access but is not a read-only confinement boundary.
