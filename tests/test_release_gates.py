...
def _write_sdist(path: Path, *, extra: dict[str, bytes] | None = None) -> None:
    ...
    with tarfile.open(path, "w:gz") as bundle:
        for name, data in sorted(files.items()):
            info = tarfile.TarInfo(f"atlas-0.1.0/{name}")
            info.size = len(data)
...
def test_ci_and_release_use_shared_gates_without_manual_active_config():
    ...
    # 旧的 RC 专用 release.yml 已退役;稳定版发布走 release-assets.yml,
    # 且必须在打包前跑完与 CI 相同的完整验证链(不再只打包不测试)。
    release = (ROOT / ".github/workflows/release-assets.yml").read_text(encoding="utf-8")
...
def test_release_assets_bind_build_to_stable_tag_push_and_reserved_release():
    ...
    assert "CONTEXT_SHA: ${{ github.sha }}" in workflow
    assert "does not match pyproject version" in workflow
    assert "--method POST" in workflow
    assert "release_id=$($created.id)" in workflow
    assert "--hostname uploads.github.com" in workflow
    assert "releases/$env:RELEASE_ID/assets" in workflow
    assert "-F draft=false" in workflow
    assert "$release.draft" in workflow
    assert "$release.tag_name -ne $env:RELEASE_TAG" in workflow
    assert workflow.count("exit 0") >= 3
    assert "tag_name: v0.1.0" not in workflow
    ...
    def fake_run(command, *, cwd, env):
        ...
        output = json.dumps({
            ...
            "tools": [
                ...
                "atlas_list_workflows", "atlas_resume_run",
